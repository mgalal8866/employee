  ...
  <v-card elevation="5">
    ...
    <v-simple-table class="mt-3" v-slot:default>
      <template elevation="5">
        ...
        <tbody>
          <tr v-for="item in itemposition" :key="item.id">
            <td>{{ item.name }}</td>
            <td>
              <v-menu offset-y>
                <template v-slot:activator="{ on, attrs }">
                ...
                <v-list>
                  <v-list>
                    ...
                    <v-list-item  @click="deleteposition(item.id)">
...
export default {
  ...
  computed: {
    ...mapState({itemposition: state=> state.position.position}),
  ...
  methods: {
    ...mapActions("position", ["getposition","newposition","editposition","deleteposition"]),
    submitForm() {
            if(this.createoredit == 'create'){
                this.newposition({nameall:this.nameall});
                this.dialog=false;
            }else
            {
                this.editposition({id:this.id,nameall:this.nameall});
                this.dialog=false;
            }
        },
        createoredit1(){
        if( this.createoredit == 'create'){
            this.dialogtitle='New Position';
            this.nameall='';
            this.dialog=true;
        }else
        {
            this.dialogtitle='Edit Position';
            this.dialog=true;
        }
    }
    }
